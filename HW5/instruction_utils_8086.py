...
def mov_between_segs_regs_and_memory(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Address, Address, int]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
    ...
    src_val = mem_layout.registers[src_reg['pos']] 
    dest_reg = register_lables[dest_decode.val]
    
    old_reg_val = mem_layout.registers[dest_reg['pos']]
    mem_layout.registers[dest_reg['pos']] = src_val
    sim_out = f"{output} ; {dest_decode}:{old_reg_val:#06x}->{mem_layout.registers[dest_reg['pos']]:#06x} \n"
    print(sim_out)
...
# INSTRUCTION SETS 
def mov_between_mem_and_reg(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
       Reference page: 4-22 MOV instruction
    """
    ...
    if dest_nbytes == 2: 
        ...
        mem_layout.registers[dest_reg['pos']] = (old_reg_val & ~(bit_mask))  + (src_val & bit_mask)
    
    sim_out = f"{output} ; {dest_decode}:{old_reg_val:#06x}->{mem_layout.registers[dest_reg['pos']]:#06x} \n"
    print(sim_out)
    return output, new_offset, sim_out

def mov_immediate_to_reg_or_memory(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
       Reference page: 4-22 MOV instruction
    """
    '''
...
def mov_immediate_to_reg(buf:bytes, buf_off:int,  mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
       Reference page: 4-22 MOV instruction
    """
    '''
    ...
    # Simming
    dest_reg = register_lables[dest_decode.val]
    old_reg_value = mem_layout.registers[dest_reg['pos']]
    dest_nbytes = dest_reg["bytes"]
    dest_is_high = dest_reg["is_high"]
    sim_out=""
    ...
    if dest_nbytes == 2:
        ...
        bit_mask = 0xff00 if dest_is_high else 0x00ff
        src_val = src_val << 8 if dest_is_high else src_val
        mem_layout.registers[dest_reg['pos']] = (old_reg_value & ~(bit_mask))  + (src_val & bit_mask)
        
    sim_out = f"{output} ; {dest_decode}:{old_reg_value:#06x}->{mem_layout.registers[dest_reg['pos']]:#06x} \n"
    print(sim_out) 
    return output, new_offset, sim_out


def mov_mem_to_accum(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
       Reference page: 4-22 MOV instruction
    """
...
def mov_accum_to_mem(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    """Reference Manual: Intel 8086 Family User's Manual October 1979
       Reference page: 4-22 MOV instruction
    """
...
def arith_sim(src_decode:Address, dest_decode:Address, mem_layout:MemoryLayout8086, arith_opcode: bytes)->str:
    # Simming
    if src_decode.is_memory or dest_decode.is_memory:
        return  '\n'
    sim_out = ""
    ...
    sim_out = f"; {reg_activity}{flags_str} \n"
    
    return sim_out

def arith_immediate_to_register_memory(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    '''
    Byte 1
    ...
    sim_out = arith_sim(src_decode, dest_decode, mem_layout, arith_opcode)
    sim_out = str(output) + sim_out
    
    return output, new_offset, sim_out


def add_between_register_memory(buf:bytes, buf_off:int,mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    ...
    sim_out = str(output)
    sim_out += arith_sim(src_decode,dest_decode, mem_layout, 0b000)
    return output, new_offset, sim_out


def add_immediate_to_accumulator(buf:bytes, buf_off:int,mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
...
def sub_between_register_memory(buf:bytes, buf_off:int,mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    ...
    if src_decode.is_memory or dest_decode.is_memory:
        return output, new_offset, str(output) + '\n'
    sim_out = str(output)
    sim_out += arith_sim(src_decode,dest_decode, mem_layout, 0b101)

    return output, new_offset, sim_out


def sub_immediate_from_accumulator(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]: 
...
def cmp_between_register_memory(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
    ...
    sim_out = str(output)
    sim_out += arith_sim(src_decode,dest_decode, mem_layout, 0b111)
    return output, new_offset, sim_out


def cmp_immediate_from_accumulator(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple[Instruction, int, str]:
...
def jmp_unconditional(buf:bytes, buf_off:int, mem_layout:MemoryLayout8086)->t.Tuple(str, int):
    '''
    Byte 1
    Jump OpCode                                             - 8 bits
