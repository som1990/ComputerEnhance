...
In this module we add support for Arithmetic operators: ADD, SUB and CMP as well as Jump instructions
...
def write_out_assembly(out_path: str, output: str):
...
path =os.path.join(dirname, 'HW_Lst_41_add_sub_cmp_jnz')
#path =os.path.join(dirname, 'HW_lst_42_completionist')
out_path = str(path) + '_out.asm'
output = disassemble_CPU8086(path)

write_out_assembly(out_path, output)
